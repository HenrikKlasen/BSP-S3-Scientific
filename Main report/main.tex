...
As already discussed in \ref{satellite}, the satellite cannot be simplified to static orbits, as it is very much influenced by the bigger celestial bodies. 
% $$ \vec{v_{n}}(t) = (\gamma \cdot \sum_{i=1}^{10}{\frac{m_{i}}{\vec{r_{i}}^{2}}}) \cdot (\Delta t)^{2} + \vec{v}_{n-1} $$
\subsection{Assessment (0/150 words)}
