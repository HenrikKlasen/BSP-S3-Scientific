...
\begin{itemize}
	\item \keys{W} – Forwards movement
	\item \keys{A} – Left movement
	\item \keys{S} – Backwards movement
	\item \keys{D} – Right movement
...
\end{itemize}
Furthermore the  satellite will be controllable by the user, to ensure, that the user gets a better understanding of the complexity of navigating in a three-dimensional space under the influence of gravitational pull to other celestial body's. 
...
\label{req-sec-computational-model}
...
\end{enumerate}
\subsection{Assessment (178/150 words)}
The results produced in this part of the project fulfil the requirements (\ref{req-sec-computational-model}). Specifically, the requirements were the following:
\begin{itemize}
	\item Explain the concept of a computational model: This is done in the production section and mentions that it is a mathematical and algorithmic construct describing a real world environment.\\
	\emph{Critical view}: As computational models are quite a wide field of research, the results are heavily broken down to not exceed the scope of the project.
	\item Give instructions on how to construct a computational model: The subsections of the production section give a detailed overview on the four main aspects for computational modelling. Namely: 
	\begin{itemize}
		\item Observation
	...
	\end{itemize}
	After detailed explanations on these four parts, the fifth subsection aims on giving a four step guide on computational models, with given main questions to consider when doing a computational model.\\
	\emph{Critical view}: as already mentioned for the last point, the topic of computational models is a very wide field. Furthermore is it difficult to find general resources on computational modelling, which made this part of the project especially difficult.
\end{itemize}
...
This section requires to execute steps 1 and 2 from \ref{Guide} in order of finding a computational model for the solar system. \\
...
\subsection{Assessment (177/150 words)}
The requirements of this section are close to be fulfilled. The only requirement partially missing is the point, why the observed behaviour is important. But this is not that crucial, since the next section transforms the observations into mathematics, which is basically showing, why the observations are important. All other requirements are fulfilled as follows:
\begin{itemize}
	\item Execute steps 1 and 2 of the guide given in \ref{Guide}: This is done in this section with the subsections "Observable behaviour" and "Problem definition. \\ \emph{Critical view}: Since the solar system has a lot of laws of physics, external objects and small internal objects, such as asteroids, the behaviour observation is mainly focussed on the major objects (Planets and Sun). Given this fact, the model build on these observations will have some degree of inaccuracy.
	\item Problem definition: The problem definition contains all necessary behaviour and properties we want to simulate. \\
	\emph{Critical view}: The aspects mentioned in the problem definition do not contain all aspects of the solar system, which is nearly impossible. Given this, the model has a bit of error introduced.
\end{itemize} 
...
\subsection{Assessment (205/150 words)}
The requirements for this section are fulfilled, since all necessary formulas are funded on arguments. 
\begin{itemize}
	\item Sun: The Sun is simplified to a static object following the observations from \ref{observations}.\\ 
	\emph{Critical view}: As the sun is not fully static in the real-world scenario, it is in that point not realistic. Since the movement is really small, it suffices for the model developed in the technical part.
	\item Planets: The planet orbits have been simplified to static orbits in this project.\\
	\emph{Critical view}: Since the planet orbits are not static, this introduces an error on the planet orbits. With that, the position of the planet in the real world scenario is not accurate, which also introduces an error in the satellite force vector computations. But since computations concerning vectors are computationally expensive and require more resources, this decision was made.
	\item Satellite: The satellite is an object under the gravitational influence of all other objects in the simulated system. This fulfils all requirements of the observation section.\\
	\emph{Critical view}: Since there is an error introduced for the planets, this error has an effect on the satellite, introducing another error. But within the given resources for computations, this model is still quite usable for rough estimations for the satellite trajectories.
...
The idea behind time warp in this project is, to speed up the simulation, while still having the same physics apply to them. This basically means, that any formula in dependence on $\Delta t$ (time-difference) will have to have a factor f to apply the time warp. \\
...
$$ \vec{v_{n}}(t) = (\gamma \cdot \sum_{i=1}^{10}{\frac{m_{i}}{\vec{r_{i}}^{2}}}) \cdot (\Delta t)^{2} + \vec{v}_{n-1} $$
Given this formula, we can observe, that we require each celestial bodies position for calculating the distance $r_{i}$ and the celestial bodies mass $m_{i}$, as well as the satellite's previous velocity vector $\vec{v}_{n-1}$.
\subsubsection{Determining celestial bodies, fetching data and finding constants} \hfill \\
Our model has ten celestial body's (the sun, all planets and Pluto). Since we want to simplify our algorithm as much as possible, we can save all celestial body's in a game object array, over which we can iterate. \\
