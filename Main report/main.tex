...
Each computational model tries to emulate a real world scenario as close as possible. To accomplish this, the model needs formulas describing the behaviour as good as possible. As it is sometimes quite difficult to find general formulas, e.g. for the n-body-physics problem, it often suffices to use approximation formulas to model. Often, these approximations make the program more efficient, as they usually do not take all environmental factors into account. \\
Furthermore it usually decreases the complexity of the model to use approximation formulas, as simplifications help to decrease the algorithmic complexity. As of today most models at large research institutions do not really have a constraint on computational power, as they usually run in large scale supercomputers. But some models also require to run on lower end machines, such as desktop or laptop work stations, which requires the program to be way more efficient to be usable. This computational constraint can also be realized by using as few high-complexity calculations as possible, which often implies to use approximation formulas.

\subsubsection{Algorithmic design}
Assume, there is a good (approximation) formula describing the real world behaviour of the environment we want to simulate. \\
