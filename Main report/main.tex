...
As there are only few resources providing a general overview on computational models, this section of the report will mainly be consisting of information found in 
...
For finding results to the scientific subtopic, it is necessary to check the data and equations given in literature. Mainly the equations describing planet orbits and gravitation are necessary to find a way to transform these equations to the corresponding algorithms. Furthermore will literature, such as the sources be used to find out \emph{how} to find answers and how physics can be used in a simplified way. Furthermore will \ref{sec-computational-model} be used to point out the significant formulas and algorithms which define this computational model. This will make this section an example on how to step by step build a computational model.
\subsection{Production (0/400 words)}
