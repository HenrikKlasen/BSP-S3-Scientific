...
\label{CamMov}
To simplify the above formula in words: Instead of moving in an absolute coordinate system, relative to the origin, we define the camera as the origin of a new coordinate system, where x, y and z are always forwards/backwards, sidewards and up-/downwards. This means, that controlling the camera is a bit more natural, as it always going exactly where the user is expecting it to move.
